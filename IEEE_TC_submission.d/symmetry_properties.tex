...
We now show that the converse problem, that is, determining whether a given
...
\end{enumerate}
\end{observation}
We now specify an iterative algorithm for determining whether 
$f$ is an NCF, and if so, constructing a simplified 
NCF representation (defined in Section~\ref{sec:intro}) for $f$.
In iteration $j$, the algorithm uses a set $X_j$ of remaining variables,
table $T_j$ with $\mu_j$ rows, and the number of groups $g_j$.
Initially (i.e., $j = 1$), $X_1$ = $\{x_1, \ldots, x_n\}$,
$T_1$ is the given table $T$ for $f$, 
$\mu_1 = \mu$ (the number of rows of $T$) and $g_j = r$ (the number of
symmetry groups of $f$) and the NCF representation for $f$ is empty.
The algorithm executes as long as one or more symmetry groups remain.
In each iteration, the algorithm carries out the following steps.

\smallskip
\noindent
I. Use Observation~\ref{obs:rsym_canalyzing} to determine whether there is
a variable $x_i \in X_j$ is canalyzing. 
...
Given an $r$-symmetric function $f$, the problem of testing whether
$f$ is an NCF can be solved efficiently. \QED
\end{theorem}
