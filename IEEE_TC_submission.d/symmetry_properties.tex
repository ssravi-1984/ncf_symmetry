...
determine in $O(g_j \mu_j)$ = $O(r \mu_j)$ time using 
Observation~\ref{obs:rsym_canalyzing}
whether there is a canalyzing variable.
...
To see this, suppose the canalyzing variable $x_i$ found in iteration $j$
is in group $\alpha_p$, which has $m_p$ variables. 
Thus, in $T_j$, there are $m_p+1 \geq 2$ possibilities for the number of 1's 
in $\alpha_p$. 
As indicated in Substep~5 of Step~II above, in $T_{j+1}$, 
we only retain those rows of $T_j$ where all the variables 
in $\alpha_p$ have the same value.
In other words, there is only one possibility for the number of
1's in group $\alpha_p$.
...
Thus, the running time over all the $r$ iterations is bounded by
$O(r [\sum_{j=1}^{r}(\mu/2^{j-1})])$ = $O(r\mu)$.
