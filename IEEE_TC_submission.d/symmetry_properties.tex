...
At beginning of iteration $j$, $X_j$ denotes the set of remaining variables,
$T_j$ denotes the table which provides values for the function,
$\mu_j$ denotes the number of rows of $T_j$ and $\ell_j$ 
denotes the number of remaining groups.
Initially (i.e., $j = 1$), $X_1$ = $X$ = $\{x_1, \ldots, x_n\}$,
$T_1 = T$ (the given table $T$ for $f$), 
$\mu_1 = \mu$ (the number of rows of $T$) and $\ell_j = r$ (the number of
symmetry groups of $f$); further, the NCF representation for $f$ is empty.
...
\item If $\ell_{j+1} \geq 1$, start the next iteration; 
...
(This is done by considering an arbitrary variable 
from each group.)
