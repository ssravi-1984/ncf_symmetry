...
This class, introduced by \cite{Kauffman-1969}, is defined as follows.
...
was introduced later by \cite{Kauffman-etal-2003} to carry out a detailed
...
For fixed $r \geq 1$, the class of $r$-symmetric functions has been
...
In general, there are Boolean functions with $n$ variables that are properly
...
unless \textbf{P} = \cnp.)
We also present an algorithm to test whether a given
...
Paul-etal-2019,2017b-Kadelka-etal}).
Generalized versions of nested canalyzing functions where the 
variables and function values can be from a domain 
of size three or more have also been studied 
(e.g., \cite{2017a-Kadelka-etal}).
